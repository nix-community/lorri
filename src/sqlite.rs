...
pub async fn migrate_gc_roots(logger: slog::Logger, paths: Paths) -> Result<(), ExitError> {
    let conn = sqlite::Connection::open_in_memory()
        .await
        .expect("cannot open sqlite db");
    let logger2 = logger.clone();
    let mut res = conn.call_unwrap(move |conn| {
        conn.execute_batch(
        ...
        let mut stmt = conn.prepare("SELECT nix_file, last_updated from gc_roots")
            .unwrap();
        let res = stmt
            .query_map((), |row| {
                let nix_file = OsString::from_vec(row
                    .get::<_, Vec<u8>>("nix_file")
                    .unwrap());
