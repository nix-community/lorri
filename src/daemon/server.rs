...
use crate::socket::communicate::{self, DaemonInfo, StreamEvents, StreamSnapshot};
                ...
                match conn {
                    Ok(connection) => {
                        ...
                        match sock.shutdown().await {
                            Err(err) => {
                                warn!(&logger2, "Error in server accept socket shutdown()"; "error" => err)
                            }
                            Ok(()) => {}
                        }
                        drop(sock)
            ...
            let socket = {
                ...
                match communication_type {
                    CommunicationType::DaemonInfo => {
                        let mut rw = handlers::from_state_unsafe::<DaemonInfo>(read_writer_state);
                    ...
                    CommunicationType::Ping => {
                        let mut rw =
                            handlers::from_state_unsafe::<communicate::Ping>(read_writer_state);
                    ...
                    CommunicationType::StreamEvents => {
                        let mut rw = handlers::from_state_unsafe::<StreamEvents>(read_writer_state);
                    ...
                    CommunicationType::StreamSnapshot => {
                        let mut rw =
                            handlers::from_state_unsafe::<StreamSnapshot>(read_writer_state);
