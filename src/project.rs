...
impl Project {
    ...
    pub fn gc_root_path(&self) -> AbsPathBuf {
...
pub fn list_roots_migration(
    ...
    for project_gc_root_dir in project_gc_root_dirs {
        ...
        let nix_file_backlink = project_root_dir.join("gc_root").join("nix_file");
        let link = match std::fs::read_link(&nix_file_backlink) {
            Ok(a) => a,
            Err(e) => {
                warn!(
                    logger,
...
pub struct ListRootsMigration {
    ...
    /// The backlink to the nix_file (is going to be deleted after the migration)
    pub nix_file_backlink: AbsPathBuf,
