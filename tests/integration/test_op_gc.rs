...
use lorri::project::{list_roots, ListRootsSort, Project, ProjectFile};
use lorri::{AbsPathBuf, NixFile};
use std::collections::HashSet;
use std::io::Cursor;

#[tokio::test]
async fn gc() -> std::io::Result<()> {
...
#!/bin/sh
...

    {
        ...
        let roots =
            list_roots(&logger, &paths, ListRootsSort::NoSorting).expect("cannot list roots");
