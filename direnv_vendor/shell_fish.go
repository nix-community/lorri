...
const fishHook = `
    function __lorri_export_eval --on-event fish_prompt;
        "{{.SelfPath}}" export fish | source;

        if test "$lorri_fish_mode" != "disable_arrow";
            function __lorri_cd_hook --on-variable PWD;
                if test "$lorri_fish_mode" = "eval_after_arrow";
                    set -g __lorri_export_again 0;
                else;
                    "{{.SelfPath}}" export fish | source;
                end;
            end;
    ...
    function __lorri_export_eval_2 --on-event fish_preexec;
        if set -q __lorri_export_again;
            set -e __lorri_export_again;
            "{{.SelfPath}}" export fish | source;
            echo;
        end;

        functions --erase __lorri_cd_hook;
    end;
`
